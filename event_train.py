...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    # model_path = "/home/lhl/Git/yolox-pytorch-bilibili/100logs/ep100-loss3.964-val_loss4.021.pth"
    model_path = ''
    #------------------------------------------------------#
    ...
    #----------------------------------------------------#
    #   获得图片路径和标签
    #   对于EventJsonDataset，我们使用JSON文件而不是txt文件
    #----------------------------------------------------#
    ...
    checkpoint = torch.load('/home/lhl/Git/YOLOX-main/YOLOX_outputs/evdet200k_80epoch_continue/yolox_base/last_epoch_ckpt.pth')

    # 2. 从字典中提取模型的 state_dict
    # 错误信息显示，模型权重保存在一个名为 'model' 的键下。
    model_state_dict = checkpoint['model']
    torch.save(model_state_dict, './yolox_only_weights.pth')
    ...
    #---------------------------#
    #   准备使用EventJsonDataset
    #---------------------------#
    # 不需要读取txt文件，我们将直接在EventJsonDataset中使用JSON文件
    # num_train   = 1000  # 暂时设置一个合理的数值，实际使用时会被EventJsonDataset覆盖
    # num_val     = 200   # 暂时设置一个合理的数值，实际使用时会被EventJsonDataset覆盖
    ...
    if True:
        ...
        train_dataset   = EventJsonDataset(
            ...
            epoch_length=end_epoch - start_epoch,
            mosaic=True,
            train=True,  # 训练模式
            augment_ration=augment_ration,  # 整体数据马赛克比例
            mosaic_prob=mosaic_prob,  # 添加 mosaic 增强概率
            hsv_prob=hsv_prob # 添加 hsv 增强概率
        )
